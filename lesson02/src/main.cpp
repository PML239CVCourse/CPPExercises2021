...
void task2() {
    ...
    rassert(array2d.empty(), "vector a should of size 0!");
...
void task3() {
    ...
    std::cout << "введите rows и columns в пределах от 1 до 20 включительно\n";
    int rows, columns;
    cin >> rows >> columns;
    ...
    rassert(rows >= 1 && rows <= 20, "rows is out of bounds");
    rassert(columns >= 1 && columns <= 20, "columns is out of bounds");
    ...
    arr.resize(rows, vector<int>(columns));
