...
void task2() {
    ...
    rassert(array2d.size() == 0, 555555555555);
...
//    // теперь давайте добавим еще один ряд чисел но другим способом:
    ...
    rassert(row1.size() == 3, 222222222);
    rassert(array2d.size() == 2, 333333333);
...
void task3() {
    ...
    int cols;
    int rows;
    std::cout << "Please enter HEIGHT (rows) and WIDTH (cols) of a table:" << std::endl;
    std::cin >> rows >> cols; // считали оба числа из консоли
    ...
    rassert(rows < 21, "0<rows<21");
    rassert(cols < 21, "0<cols<21");
    ...
    while (true) {
        ...
        for (int j = 0; j < rows;j++){
            Oh = true;
            for (int i = 0; i< cols; i++){
                if (array[j][i]!=1) Oh = false;
            }
            if (Oh) std::cout<<"OX-XO-XO"<<std::endl;
        }
        for (int i = 0; i < rows;i++){
            Ah = true;
            for (int j = 0; j< cols; j++){
                if (array[j][i]!=1) Ah = false;
            }
            if (Ah) std::cout<<"AX-XA-XA"<<std::endl;
        }
