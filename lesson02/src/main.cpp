...
//    // TODO 26 добавьте два rassert-а - чтобы удостовериться что размеры двух динамических массивов которые мы только что изменили - совпадают с вашим ожиданием
      rassert(array2d.size() == 2,2394);
      rassert(row1.size() == 3,2395);
    std::cout << "print2DArray(array2d):" << std::endl;
...
void task3() {
    ...
    int rows, cols;
    std::cout << "Enter please rows and cols (ot 1 do 20)" << std::endl;
    std::cin >> rows >> cols;
    ...
    rassert((rows > 0 && rows < 21),2391);
    rassert((cols > 0 && cols < 21),2392);
    // TODO 34 создайте двумерный массив состоящий из rows векторов
    std::vector<std::vector<int>> array2d;
    std::vector<int> row;
    row.resize(cols);
    while(array2d.size() < rows){
    ...
    while (true) {
        ...
        if(kek == 101) std::cout << "OX-XO-XO" << std::endl;
