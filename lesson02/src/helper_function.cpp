...
void printRows(std::vector<std::vector<int>> rows) {
    for (int j = 0; j < rows.size(); ++j) { // пробегаем по каждой строке

        int n = rows[j].size(); // узнаем число элементов в текущей строке

        // TODO 24 выведите все элементы этой строчки в одну строку в консоли через пробел
        for (int i = 0; i < n; ++i) {
            int value = rows[j][i]; // взяли j-ую строку, а из нее - i-ый элемент
            // выведите в консоль это число
            std::cout<< value << " ";
            if (i== n-1) {
                std::cout<<  std::endl;
