...
cv::Mat buildHough(cv::Mat sobel) {// единственный аргумент - это результат свертки Собелем изначальной картинки
    ...
    cv::Mat accumulator(width+height, 180, CV_32FC1); // TODO подумайте какого разрешения она должна быть и поправьте ее размер
    for(int i = 0; i < 100; ++i)
        for(int j = 0; j < 100; ++j)
    ...
    for (int y0 = 0; y0 < height; ++y0) {
        for (int x0 = 0; x0 < width; ++x0) {
            ...
            for (int theta0 = 0; theta0 < max_theta; ++theta0) {
                ...
                double t_theta = (double)theta0 * PI / 180;
                ...
                float r0 = x0 * cos(t_theta) + y0 * sin(t_theta);
                ...
                rassert( theta0 >= 0, 237891731289044);
                rassert(theta0 < accumulator.cols, 237891731289045);
