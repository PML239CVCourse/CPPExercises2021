...
cv::Mat buildHough(cv::Mat sobel) {// единственный аргумент - это результат свертки Собелем изначальной картинки
    ...
    int max_r = sqrt((double)width*width+(double)height*height)+1; //
    ...
    cv::Mat accumulator(   max_r,max_theta, CV_32FC1, 0.0);
    ...
    for (int y0 = 0; y0 < height; ++y0) {
        for (int x0 = 0; x0 < width; ++x0) {
            ...
            for (int theta0 = 0; theta0 < max_theta; ++theta0) {
                ...
                float r0 = x0*cos(theta0*PI/180)+y0*sin(theta0*PI/180)-0.000001;
